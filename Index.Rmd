...
## Datasets used
My main dataset is the World Health Organization's international dataset, which includes number of COVID cases and deaths reported both daily and cumulatively, by country (WHO, 2020). This dataset is updated daily. Data collection began in mid-January and this report includes data through August 2nd, 2020.
...
# I want to stress that I spent roughly 10 hours trying to figure out how to add regression lines to facited plots, and while it is technically possible, it requires a significant formatting change because you have to add it to each graph individually. The resulting graphs look horrible when there are this many plots. So I did not add correlation constants, please forgive me.
ggplot() +
...
```{r, echo = FALSE}
# Average new deaths per capita in Countries with Non-Universal Healthcare. Compliments above plot.
knitr::kable(NonUniversalByPop %>%
              rename("Country" = "Country_name") %>%
            summarize(Mean = mean(DeathsPerCapita),
              Median = median(DeathsPerCapita),
			 			  StdDev = sd(DeathsPerCapita), .groups = 'drop'),
			 align = c('l','c','c','c','c'), 
			 digits = 5,
			 format = "html", 
			 caption = "**Table 5**: Summary of Most Recent New Deaths per Capita in Countries with Non-Universal Healthcare",
			 table.attr = "style='width:70%;'") %>% kable_styling(position = "center")
```

On the most recent day analyzed
...
```{r, echo = FALSE}
# Average new deaths per capita in Countries with Universal Healthcare. Compliments above plot.
knitr::kable(UniversalByPop %>%
              rename("Country" = "Country_name") %>%
            summarize(Mean = mean(DeathsPerCapita),
                      Median = median(DeathsPerCapita),
			 			  StdDev = sd(DeathsPerCapita), .groups = 'drop'),
			 ...
			 digits = 5,
			 format = "html", 
			 caption = "**Table 5**: Summary of Most Recent New Deaths per Capita in Countries with Universal Healthcare",
			 table.attr = "style='width:70%;'") %>% kable_styling(position = "center")
```
